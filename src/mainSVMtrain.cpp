...
int gkmTrainRun(OptsGkmTrain &opt)
{
	...
	if (opt.nfold > 1) {
		...
		gkmMsg("Cross validation (%d folds) accuracy = %g%%\n", opt.nfold, 100.0 * correct / N);
	}
	...
	if (opt.legacyPair) {
		...
		if (fo_alpha == NULL || fo_sv == NULL) { gkmMsg("\n ERROR: cannot write %s / %s\n", alphaFN.c_str(), svFN.c_str()); svm_free_and_destroy_model(&model); return 1; }
	}
	if (opt.modelFile) {
