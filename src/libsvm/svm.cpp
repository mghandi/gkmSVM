...
/* ---- gkmSVM patch (see README-gkmsvm.md): the R package must not call rand() or write to
 * stdout/stderr directly. Everything else in this file is LIBSVM 3.37 unmodified. */
#ifdef RPACKAGE
#include <R_ext/Print.h>
#include <R_ext/Random.h>
static int gkm_rand(void) { return (int)R_unif_index((double)RAND_MAX); }
static void gkm_warn(const char *fmt, ...) { char buf[BUFSIZ]; va_list ap; va_start(ap, fmt); vsnprintf(buf, BUFSIZ, fmt, ap); va_end(ap); REprintf("%s", buf); }
#define rand gkm_rand
#else
static void gkm_warn(const char *fmt, ...) { va_list ap; va_start(ap, fmt); vfprintf(stderr, fmt, ap); va_end(ap); }
#endif
/* ---- end of gkmSVM patch */
#ifdef _OPENMP
#include <omp.h>
...
static int svm_one_class_probability(const svm_problem *prob, const svm_model *model, double *prob_density_marks)
{
	...
	{
		gkm_warn("WARNING: number of positive or negative decision values <%d; too few to do a probability estimation.\n",nr_marks/2);
		ret = -1;
	}
	else
	{
		...
		for(i=0;i<param->nr_weight;i++)
		{
			...
			for(j=0;j<nr_class;j++)
				if(param->weight_label[i] == label[j])
					break;
			if(j == nr_class)
				gkm_warn("WARNING: class label %d specified in weight is not found\n", param->weight_label[i]);
			else
...
void svm_cross_validation(const svm_problem *prob, const svm_parameter *param, int nr_fold, double *target)
{
	...
	if (nr_fold > l)
	{
		gkm_warn("WARNING: # folds (%d) > # data (%d). Will use # folds = # data instead (i.e., leave-one-out cross validation)\n", nr_fold, l);
		nr_fold = l;
	}
...
double svm_get_svr_probability(const svm_model *model)
{
	...
	{
		gkm_warn("Model doesn't contain information for SVR probability inference\n");
		return 0;
	}
}
...
bool read_model_header(FILE *fp, svm_model* model)
{
	...
	while(1)
	{
		...
		else if(strcmp(cmd,"kernel_type")==0)
		{
			...
			{
				gkm_warn("unknown kernel function.\n");
				return false;
			}
		}
		...
		{
			gkm_warn("unknown text in model file: [%s]\n",cmd);
			return false;
		}
	}
...
svm_model *svm_load_model(const char *model_file_name)
{
	...
	{
		gkm_warn( "ERROR: fscanf failed to read model\n");
		setlocale(LC_ALL, old_locale);
		free(old_locale);
		free(model->rho);
		free(model->label);
